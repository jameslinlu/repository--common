...
public final class IPv6AddressHelpers {
    ...
    public static BigInteger ipToNumber(String addr) {
        ...
        if (startIndex != -1) {
            ...
            BigInteger first = ipToNumber(firstStr);
            int x = countChar(addr, ':');
            first = first.shiftLeft(16 * (7 - x)).add(ipToNumber(secondStr));
            return first;
        }
    ...
    public static String numberToIp(String ipNumber) {
        return numberToIp(new BigInteger(ipNumber));
    }

    public static String numberToIp(BigInteger ipNumber) {
        String ipString = "";
