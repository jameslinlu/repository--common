...
//            return StringUtils.join(subnets, ",");
    ...
    public static long ipToLong(String strIP) {
        //为0:0:0:0:0:0:0:1则返回0
        if (strIP != null && strIP.startsWith("0")) {
            return 0l;
        }
     ...
     * @return IP地址的数值表示
     */
    public static long ipToLong(String[] ipParts) {
        if (ipParts.length != 4) {
    ...
    public static String getMaskFromAntiMask(String antiMask) {
        String[] maskParts = antiMask.split("\\.");
        int[] masks = new int[maskParts.length];
        for (int i = 0; i < masks.length; i++) {
            masks[i] = Integer.parseInt(maskParts[i]) ^ 0xFF;
        }
        StringBuilder buf = new StringBuilder(16);
        for (int i = 0; i < masks.length; i++) {
            buf.append(".").append(Integer.toString(masks[i]));
        }
        return buf.substring(1);
    }
